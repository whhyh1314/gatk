...
public final class RDDUtils implements Serializable{
     ...
     */
    public static <T> Tuple2<JavaRDD<T>, JavaRDD<T>> split(final JavaRDD<T> input, final SerializablePredicate<T> predicate,
                                                           final boolean unpersistInputRDD) {

        final JavaRDD<T> first = input.filter(predicate::test).cache();
        final JavaRDD<T> second = input.filter(t -> !predicate.test(t)).cache();
